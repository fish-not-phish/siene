...
const sevBadge = (count: number, cls: string) =>
  count > 0 ? (
    <span className={`inline-flex items-center rounded-full px-1.5 py-0.5 text-xs font-medium ${cls}`}>{count}</span>
  ) : (
...
export default function SecurityHubPage() {
  ...
  const loadVulns = () => {
    ...
    fetch(`${baseUrl}registry/system/security?${params}`, { credentials: 'include' })
      .then(r => r.json()).then(d => setVulns(Array.isArray(d) ? d : [])).catch(() => setVulns([]))
  ...
  const loadSecrets = () => {
    ...
    const params = new URLSearchParams()
    if (projectFilter) params.set('project', projectFilter)
    fetch(`${baseUrl}registry/system/security/secrets?${params}`, { credentials: 'include' })
      .then(r => r.json()).then(d => setSecrets(Array.isArray(d) ? d : [])).catch(() => setSecrets([]))
  ...
  const loadMisconfigs = () => {
    ...
    const params = new URLSearchParams()
    if (projectFilter) params.set('project', projectFilter)
    fetch(`${baseUrl}registry/system/security/misconfigs?${params}`, { credentials: 'include' })
      .then(r => r.json()).then(d => setMisconfigs(Array.isArray(d) ? d : [])).catch(() => setMisconfigs([]))
      .finally(() => setLoadingMisconfigs(false))
  }

  useEffect(() => { loadVulns() }, [projectFilter, severityFilter])
  useEffect(() => { loadSecrets() }, [projectFilter])
  useEffect(() => { loadMisconfigs() }, [projectFilter])

  const q = search.toLowerCase()
  const filteredVulns      = vulns.filter(v => v.tag_name.toLowerCase().includes(q) || v.repository.toLowerCase().includes(q) || v.project.toLowerCase().includes(q))
  const filteredSecrets    = secrets.filter(v => v.tag_name.toLowerCase().includes(q) || v.repository.toLowerCase().includes(q) || v.project.toLowerCase().includes(q))
  const filteredMisconfigs = misconfigs.filter(v => v.tag_name.toLowerCase().includes(q) || v.repository.toLowerCase().includes(q) || v.project.toLowerCase().includes(q))
  ...
  const allProjects = [...new Set([
    ...vulns.map(v => v.project),
    ...secrets.map(v => v.project),
    ...misconfigs.map(v => v.project),
  ])].sort()

  const imageLink = (v: { project: string; repository: string; tag_name: string }) =>
    `/projects/${v.project}/repositories/${v.repository}/${v.tag_name}`
  ...
  return (
    <>
      <header className='sticky top-0 z-10 flex items-center gap-3 border-b bg-background/80 px-6 py-3 backdrop-blur'>
        ...
        <Separator orientation='vertical' className='h-4!' />
        <span className='text-muted-foreground text-sm'>Security Hub</span>
          ...
          <TabsContent value='vulnerabilities' className='mt-4 space-y-3'>
            <div className='flex items-center gap-2'>
              <Select value={severityFilter} onValueChange={setSeverityFilter}>
                <SelectTrigger className='h-8 w-36 text-sm'><SelectValue /></SelectTrigger>
                <SelectContent>{SEV_FILTER_OPTIONS.map(o => <SelectItem key={o.value} value={o.value}>{o.label}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            {loadingVulns ? (
              <div className='space-y-2'>{Array.from({ length: 6 }).map((_, i) => <Skeleton key={i} className='h-12 w-full rounded-md' />)}</div>
                  ...
                  <TableBody>
                    {filteredVulns.map(v => (
                      <TableRow key={v.tag_id}>
                        <TableCell><Link href={imageLink(v)} className='font-mono text-sm hover:underline'>{v.repository}:{v.tag_name}</Link></TableCell>
                        <TableCell><Badge variant='secondary' className='text-xs'>{v.project}</Badge></TableCell>
                ...
                <Table>
                  ...
                  <TableBody>
                    {filteredSecrets.map(v => (
                      <TableRow key={v.tag_id}>
                        <TableCell><Link href={`${imageLink(v)}?tab=secrets`} className='font-mono text-sm hover:underline'>{v.repository}:{v.tag_name}</Link></TableCell>
                        <TableCell><Badge variant='secondary' className='text-xs'>{v.project}</Badge></TableCell>
                ...
                <Table>
                  ...
                  <TableBody>
                    {filteredMisconfigs.map(v => (
                      <TableRow key={v.tag_id}>
                        <TableCell><Link href={`${imageLink(v)}?tab=misconfigs`} className='font-mono text-sm hover:underline'>{v.repository}:{v.tag_name}</Link></TableCell>
                        <TableCell><Badge variant='secondary' className='text-xs'>{v.project}</Badge></TableCell>
